...
auto-upgrade-cronjob-k8s-image: auto-upgrade-cronjob-k8s-clean
	...
	if [[ $(arch) == "amd64" || $(arch) == "ppc64el" || $(arch) == "arm64" ]]; then \
		echo | pwd && cd $(ANAX_K8S_CONTAINER_DIR) && echo | pwd && docker build $(DOCKER_MAYBE_CACHE) $(CRONJOB_AUTO_UPGRADE_K8S_IMAGE_LABELS) -t $(CRONJOB_AUTO_UPGRADE_K8S_IMAGE_STG) -f Dockerfile.ubi.auto-upgrade-cron.$(arch) .; \
	fi
